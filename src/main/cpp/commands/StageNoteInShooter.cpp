...
StageNoteInShooter::StageNoteInShooter( SwerveDriveSubsystem* swerve, ShooterSubsystem* shooter, 
...
void StageNoteInShooter::Initialize() {
  DataLogger::GetInstance().Send( "Command/StageNoteInShooter", true );
...
void StageNoteInShooter::Execute() {
  ...
  //    m_shooter->GoToAngle( targetAngle );
      ...
      //   fmt::print( "StageNoteInShooter(), pitch({}), Shooter({}), Arm({}), Wrist({})\n", t_pitch, m_shooter->GetAngle(),
      //                m_shooter->GetShooter_ArmAngle(), 180_deg - m_shooter->GetAngle()  );
      //   readyToShoot = true;
...
void StageNoteInShooter::End(bool interrupted) {
