...
void ShootNoteTargeting::Execute() {
  ...
  if( !readyToShoot ) {
    ...
    if( dist_to_speaker < 7_m  ) {
      units::degree_t azimuthAngle = units::math::atan2( 78_in, dist_to_speaker );
      units::degree_t shooterAngle = azimuthAngle + azimuthCorrection.lookup( dist_to_speaker.value() ) * 1_deg;
