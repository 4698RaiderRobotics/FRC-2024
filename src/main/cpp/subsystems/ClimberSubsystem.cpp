    ...
    m_climberMotor{deviceIDs::kClimberID, rev::CANSparkFlex::MotorType::kBrushless}
{
    ...
    DataLogger::GetInstance().SendNT( "ClimberSubsys/isRaising", isRaising);
};

// This method will be called once per scheduler run
void ClimberSubsystem::Periodic() {

    DataLogger::GetInstance().SendNT( "ClimberSubsys/Climber Height (in)", GetHeight().value());
    ...
    DataLogger::GetInstance().SendNT( "ClimberSubsys/Climber Current", m_climberMotor.GetOutputCurrent());
...
void ClimberSubsystem::SetSpeed(double speed) {
    if( isHoming ) {
        if( fabs( speed ) < 0.01 ) {
            ...
            DataLogger::GetInstance().SendNT( "ClimberSubsys/isHoming", isHoming);
            DataLogger::GetInstance().SendNT( "ClimberSubsys/isZeroed", isZeroed);
            DataLogger::GetInstance().Log( "ClimberSubsystem::SetSpeed() -- Homing interrupted by Nudge.\n" ); 
        }
    }
